...
const reducer = (state = defaultState, action) => {
  ...
  switch (action.type) {
    case "address/add":
      ...
      let isAdressAlreadyExist = state.addresses.find(x=>x.id=== action.payload?.id)
      return isAdressAlreadyExist ? state : { ...state, addresses:[...state.addresses, action.payload]};
    case "address/remove":
      ...
      let newAddressList = state.addresses.filter(x=>x.id!== action.payload)
